...
var cadeiras = [{"mod":"Sample Module #1","form":" Sample Formador #1"}, {"mod":" Sample Module #2","form":"Sample Formador #2"}]
...
function weekNames(date){
	...
	switch(date.getDay()){
		...
		case 3: return "Wedenesday";breaK;
...
function validateToken(){
	...
	var url = 'http://www.your-url-here.com/validate.php?token=' + document.forms['validate']['token'].value ;
...
function loadSchedule(start,end){
	...
	var request = new XMLHttpRequest();
	//var url = 'http://www.yoururl.com/getHorario.php?token='+ localStorage.getItem('token')+'&start='+ date_s +'&end='+ date_e;

	var url = 'http://www.your-url-here.com/getHorario.php?token='+ localStorage.getItem('token');
	request.onreadystatechange = function(){
		if(request.status == 200 && request.readyState == 4){
...
function parseSecret(){
	...
	var url = 'http://www.your-url-here.com/validate.php?token=' + token;
